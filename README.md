...
`ASR_PUBLIC_ORIGIN` 必须是百炼能够访问的本站地址，用于生成仅在任务期间有效的随机录音取件链接。服务会用 FFmpeg 生成单声道 ASR 副本并开启百炼说话人分离，原始上传文件保持不变；转写按“说话人 1、说话人 2……”组织连续对话。录音文件、原始转写和总结保存在 `data` 持久卷中，默认只有管理员可见；管理员可单独公开总结。长录音会先分段提取招聘事实，再合并为 Markdown 总结，并要求模型过滤无关声音、标明缺失或听不清的部分。管理员详情保留分阶段处理日志和受控的错误堆栈摘要，便于定位外部服务故障。
