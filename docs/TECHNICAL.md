...
当前 WSL 部署位于 `/home/zhihongpan/services/dapan-job-board`，使用 `127.0.0.1:58112`。`deploy/systemd` 保存开机启动与每日备份单元，`deploy/nginx/job.dapanclaw.top.conf` 保存 AWS 入口反代模板。Windows 计划任务 `Dapan Job Board WSL Startup` 在用户登录时启动 Ubuntu，使 WSL systemd 接着拉起 Compose；源腾讯云容器保持停止，作为迁移后的短期回滚副本。
